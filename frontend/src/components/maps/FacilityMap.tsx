...
const FacilityPopup = React.memo(function FacilityPopup({
  ...
  if (popupSide === 'bottom') {
    const maxLeft = windowWidth - popupWidth - POPUP_EDGE_PADDING;
    left = Math.min(Math.max(baseLeft, POPUP_EDGE_PADDING), maxLeft);
    arrowLeft = popupPos.x - left;
  }
  return (
    <div
      ref={popupRef}
...
export function FacilityMap(props: StateFacilityProps) {
  ...
  const checkPopupSide = useCallback(
    (x: number) => {
      ...
      if (mapRef.current) {
        ...
        if (x + maxWidth > rect.width - POPUP_EDGE_PADDING && x - minWidth < POPUP_EDGE_PADDING) {
          setPopupSide('bottom');
        } else if (x + maxWidth > rect.width - POPUP_EDGE_PADDING) {
          setPopupSide('left');
        } else {
          setPopupSide('right');
        }
  ...
  useEffect(() => {
    if (popupRef.current && mapRef.current) {
      ...
      const EDGE_BUFFER = POPUP_EDGE_PADDING;
      if (isSmallScreen) {
        setPopupSide('bottom');
      } else if (
